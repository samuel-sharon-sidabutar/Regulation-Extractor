...
# ==========================================
# We now use a single unified workspace folder
...
def prompt_overwrite(filepath):
    """Checks if a file exists and asks the user what to do."""
    if os.path.exists(filepath):
        while True:
            ...
            if choice in ['y', 'n', 'c']:
                return choice
    return 'y' # If it doesn't exist, 'y' means proceed with writing.
...
def convert_md_to_csv(md_path, csv_path, md_content=None):
    print(f"  -> Sending {os.path.basename(md_path)} to Gemini...")
    ...
    # 1. We create a continuous "Chat" instead of a one-off request
    chat = client.chats.create(
        model='gemini-3.5-flash',
        config=types.GenerateContentConfig(
            system_instruction=MASTER_PROMPT,
            max_output_tokens=8192, # Maximum allowed output per turn
            temperature=0.1, 
        )
    )

    # 2. Send the initial massive document
    response = chat.send_message(f"Process this document strictly according to system instructions:\n\n{md_content}")
    
    # Start saving the text
    full_csv_text = response.text

    # 3. The "Auto-Continue" Loop
    while hasattr(response.candidates[0], 'finish_reason') and str(response.candidates[0].finish_reason) == 'MAX_TOKENS':
        print("  -> Output limit reached. Automatically telling the AI to continue...")
        
        # Aggressive follow-up prompt to prevent formatting drift
        response = chat.send_message(
            "Continue exactly where you left off. "
            "DO NOT use bullet points or lists. "
            "You MUST continue outputting raw, flat Pipe-Separated Values (PSV) rows with exactly 14 columns. "
            "Do not repeat the header row."
        )
        
        # Stitch the new text onto the bottom of the old text
        full_csv_text += "\n" + response.text

    # 4. Save the fully assembled file
...
def main():
    ...
    # Look for files in the unified WORK_DIR
    if action in ['1', '3']:
    ...
    for file_path in selected_files:
        ...
        try:
            if action == '1':
                ...
                if choice == 'c': break
                elif choice == 'y': convert_md_to_csv(file_path, csv_path)
                ...
                if md_choice == 'c': 
                    ...
                    md_content = convert_pdf_to_md(file_path, md_path)
                else: 
                ...
                if csv_choice == 'c': 
                    ...
                    convert_md_to_csv(md_path, csv_path, md_content=md_content)
                else: 
